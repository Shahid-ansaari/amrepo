...
export default function LaborColonyPage() {
  const API_ENDPOINT = process.env.NEXT_PUBLIC_API_ENDPOINT || "http://localhost:4000/api/leads";

  const product = {
    name: "Labor Colony",
    subtitle: "Durable, portable labor colonies — ideal for worker accommodation and temporary housing.",
    mainImage: "/assets/hero/image (1).webp",
    images: [
      "/assets/products/labor-colony/labor1.jpg",
      "/assets/products/labor-colony/labor2.jpg",
      "/assets/products/labor-colony/labor3.jpg",
      "/assets/products/labor-colony/labor4.jpg",
      "/assets/products/labor-colony/labor5.jpg",
      "/assets/products/labor-colony/labor6.jpg",
    ],
    features: [
      { feature: "Sizes", benefit: "Custom sizes available: 10x10, 12x12, 15x10, and modular options" },
      { feature: "Build", benefit: "Galvanized steel frame + insulated panels for durability and comfort" },
      { feature: "Delivery", benefit: "Pre-built units, tested, and quick to install on-site" },
    ],
    highlights: [
      "Ideal for worker accommodation at construction sites or remote projects",
      "Energy-efficient insulated panels for year-round comfort",
      "Fully equipped interiors: beds, storage, electrical, and lighting",
    ],
  };

  const faqs = [
    { q: "How fast can I get a labor colony?", a: "Standard units: 7–15 days. Custom specifications: timeline provided on enquiry." },
    { q: "Can colonies be relocated?", a: "Yes, designed for mobility. We provide re-installation support." },
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mt-6 text-gray-800">
            {activeTab==="details" && (
              <div className="space-y-4">
                <p className="font-semibold">Labor colonies provide durable, comfortable, and portable living spaces for workers at construction sites and remote projects.</p>
                ...
                <p className="font-medium">Pain → Temporary housing is slow and expensive. Solution → Labor colonies provide instant accommodation at lower cost. Trust → Used across construction sites, industrial projects, and remote locations with after-sales support.</p>
              </div>
            )}
            {activeTab==="specs" && (
              <div className="space-y-4">
            ...
            {activeTab==="faq" && (
              <div className="space-y-3">
                {faqs.map((f,i)=>(<div key={i} className="border rounded">
            ...
            {activeTab==="images" && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {product.images.map((img,i)=>(<div key={i} className="rounded overflow-hidden cursor-pointer" onClick={()=>{setActiveImage(img); setZoom(true); window.scrollTo({top:0,behavior:'smooth'})}}>
        ...
        <section className="mt-6">
          ...
          <div className="flex gap-3 overflow-x-auto py-2">
            {product.images.map((img,i)=>(<button key={i} onClick={()=>{setActiveImage(img); setZoom(true)}} className="min-w-[160px] rounded overflow-hidden border">
        ...
        <section className="mt-10 bg-white p-6 rounded max-w-3xl mx-auto">
          <h2 className="text-2xl font-bold text-center">Get a quote for Labor Colony</h2>
          <p className="text-center text-gray-700 mt-2">Share your requirements — size, interior, number of units — and we’ll respond with options and pricing.</p>
        ...
        <section className="mt-8 text-center">
          <p className="text-gray-700">Trusted by businesses, construction sites, and remote facilities nationwide — with logistics and after-sales support.</p>
        </section>
